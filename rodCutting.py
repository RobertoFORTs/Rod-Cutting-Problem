...
def dynamicRodCutting(p, n):  # Bottom-Up
    ...
    for j in range(1, n + 1):  # Loop para cada tamanho possível da tora
        ...
        for i in range(1, j + 1):  # Loop para cada possível corte da tora
            ...
            q = max(q, p[i - 1] + r[
                j - i])  # Calcula o valor máximo entre o valor do corte atual e a combinação de cortes anteriores
...
ax1.plot(df['n'], df['tDP'], color=color, label='Tempo DP')
ax1.tick_params(axis='y', labelcolor=color)

ax2 = ax1.twinx()
color = 'tab:blue'
ax2.set_ylabel('tGreedy', color=color)
ax2.plot(df['n'], df['tGreedy'], color=color, label='Tempo Greedy')
ax2.tick_params(axis='y', labelcolor=color)

# Ajuste dos limites dos eixos y para melhor visualização
ax1.set_ylim([0, max(df['tDP'])])
ax2.set_ylim([0, max(df['tDP'])])
...
ax3.plot(df['n'], df['vDP'], color=color, label='Valor DP')
ax3.tick_params(axis='y', labelcolor=color)

ax4 = ax3.twinx()
color = 'tab:blue'
ax4.set_ylabel('vGreedy', color=color)
ax4.plot(df['n'], df['vGreedy'], color=color, label='Valor Greedy')
ax4.tick_params(axis='y', labelcolor=color)

# Ajuste dos limites dos eixos y para melhor visualização
ax3.set_ylim([0, max(df['vDP'])])
ax4.set_ylim([0, max(df['vDP'])])
